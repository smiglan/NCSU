...
class Network(object):
    ...
    def large_weight_initializer(self):
        ...
        """
        self.biases = [np.random.randn(y, 1) for y in self.sizes[1:]]
        self.weightsT = [np.random.randn(y, x)
                        for x, y in zip(self.sizes[:-1], self.sizes[1:])]
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        self.weightsT = [(1-eta*(lmbda/n))*w-(eta/len(mini_batch))*nw
                        for w, nw in zip(self.weightsT, nabla_wT)]
    ...
    def gradient_check(self, training_data, layer_id, unit_id, weight_id):
        ...
        fa = delta_nabla_wT[layer_id][unit_id][weight_id]
        # small number h
        h = 1e-5
        # compute f(x + h)
        o_weight = self.weightsT[layer_id][unit_id][weight_id]
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        cost += 0.5*(lmbda/len(data))*sum(
            np.linalg.norm(w)**2 for w in self.weightsT)
        return cost

    def save(self, filename):
        """Save the neural network to the file ``filename``."""
        data = {"sizes": self.sizes,
                "weights": [w.tolist() for w in self.weightsT],
